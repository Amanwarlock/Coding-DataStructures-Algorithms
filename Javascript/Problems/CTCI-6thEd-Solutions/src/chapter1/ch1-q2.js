...
// JS string char to ascii - str.charCodeAt(i)
function isPermutation(str1, str2){
    ...
    // for str1
    for(let i=0; i<str1.length; i++){
        if(isNaN(occurances[str1[i]])) occurances[str1[i]] = 0;
        occurances[str1[i]] ++;
    }

    for(let j=0; j<str2.length; j++){
        if(isNaN(occurances[str2[j]])) occurances[str2[j]] = 0;
        occurances[str2[j]] --;
        if(occurances[str2[j]] < 0) return false;
    }
...
console.log("Result ", isPermutation("apple", "papel"));